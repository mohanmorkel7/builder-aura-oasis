...
export class DepartmentService {
  ...
  static async getUserDepartmentByEmail(email: string): Promise<UserDepartmentInfo | null> {
    try {
      const query = `
        SELECT 
          u.id as user_id,
      ...
      return {
        ...
        permissions: [...new Set([...deptPermissions, ...additionalPermissions])],
  ...
  static async getUserDepartmentBySSOId(ssoId: string): Promise<UserDepartmentInfo | null> {
    try {
      const query = `
        SELECT 
          u.id as user_id,
  ...
  static async createOrUpdateSSOUser(ssoUser: any): Promise<UserDepartmentInfo | null> {
    try {
      // Find user in our department mapping
      const userMapping = userDepartments.users.find(u => u.email === ssoUser.mail);
      ...
      const existingUser = await pool.query('SELECT id FROM users WHERE email = $1', [ssoUser.mail]);
