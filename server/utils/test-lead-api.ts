...
export const sampleLeadData = {
  ...
  project_description: "Building a comprehensive e-commerce platform with advanced features",
  project_budget: 150000,
  project_timeline: "6-8 months",
  project_requirements: "React, Node.js, PostgreSQL, payment integration, mobile responsive",
  ...
  probability: 75,
  notes: "High priority lead with strong technical requirements. Good fit for our solutions.",
...
export const expectedDatabaseColumns = [
  // Primary fields
  'id', 'lead_id', 'created_at', 'updated_at',
  ...
  // Client Information
  'client_name', 'client_type', 'company', 'company_location', 'category', 'country', 
  'contact_person', 'email', 'phone', 'industry', 'company_size',
  
  // Contact Information
  'contacts',
  
  // Additional Information
  'status', 'priority', 'expected_close_date', 'probability', 'notes',
  
  // Metadata
  'created_by', 'assigned_to'
];

export function validateLeadData(leadData: any): { isValid: boolean; missingFields: string[]; errors: string[] } {
  const missingFields: string[] = [];
  const errors: string[] = [];

  // Check required fields
  const requiredFields = ['client_name', 'contact_person', 'email', 'lead_source', 'created_by'];
  ...
  if (leadData.expected_daily_txn_volume !== undefined) {
    ...
    if (isNaN(volume) || volume < 0) {
      errors.push('Expected daily transaction volume must be a positive number');
    }
  }

  // Validate enum values
  const validLeadSources = ['email', 'social-media', 'phone', 'website', 'referral', 'cold-call', 'event', 'other'];
  ...
  if (leadData.priority_level && !validPriorityLevels.includes(leadData.priority_level)) {
  ...
  if (leadData.commercial_pricing && !Array.isArray(leadData.commercial_pricing)) {
  ...
  if (leadData.commercial_pricing && Array.isArray(leadData.commercial_pricing)) {
    for (let i = 0; i < leadData.commercial_pricing.length; i++) {
      ...
      if (!pricing.solution || typeof pricing.solution !== 'string') {
        errors.push(`Commercial pricing item ${i + 1}: solution is required and must be a string`);
      }
      if (pricing.value === undefined || isNaN(Number(pricing.value))) {
        errors.push(`Commercial pricing item ${i + 1}: value is required and must be a number`);
      }
      if (!pricing.unit || !['paisa', 'cents'].includes(pricing.unit)) {
        errors.push(`Commercial pricing item ${i + 1}: unit must be either 'paisa' or 'cents'`);
      }
      if (!pricing.currency || !['INR', 'USD', 'Dubai'].includes(pricing.currency)) {
        errors.push(`Commercial pricing item ${i + 1}: currency must be either 'INR', 'USD', or 'Dubai'`);
      }
    }
  }
...
export function logApiTestResults(testName: string, success: boolean, data?: any, error?: any) {
