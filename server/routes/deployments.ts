...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    if (!deploymentData.product_id || !deploymentData.version || !deploymentData.environment || !deploymentData.created_by) {
      return res.status(400).json({ error: 'Missing required fields: product_id, version, environment, created_by' });
    }

    // Validate environment
    if (!['development', 'staging', 'qa', 'production'].includes(deploymentData.environment)) {
...
router.put('/:id', async (req: Request, res: Response) => {
  try {
    ...
    if (deploymentData.environment && !['development', 'staging', 'qa', 'production'].includes(deploymentData.environment)) {
      return res.status(400).json({ error: 'Invalid environment value' });
    }

    // Validate status if provided
    if (deploymentData.status && !['pending', 'scheduled', 'in_progress', 'completed', 'failed', 'cancelled'].includes(deploymentData.status)) {
...
router.patch('/:id/status', async (req: Request, res: Response) => {
  try {
    ...
    if (!status || !['pending', 'scheduled', 'in_progress', 'completed', 'failed', 'cancelled'].includes(status)) {
      return res.status(400).json({ error: 'Invalid status value' });
    }
