-- =====================================================
-- BANANI APP UNIFIED DATABASE SCHEMA
...
CREATE TABLE IF NOT EXISTS onboarding_templates (
    ...
    category_id INTEGER REFERENCES template_categories(id),
    template_type_id INTEGER REFERENCES template_types(id),
    tags TEXT[],
...
CREATE TABLE IF NOT EXISTS template_steps (
    ...
    step_category_id INTEGER REFERENCES step_categories(id),
    assigned_role VARCHAR(50), -- which role typically handles this step
    required_documents TEXT[], -- documents required for this step
    approval_required BOOLEAN DEFAULT FALSE,
    parallel_execution BOOLEAN DEFAULT FALSE, -- can be done parallel with other steps
    dependencies TEXT[], -- step IDs this step depends on
    custom_fields JSONB,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS ticket_comments (
    ...
    comment_type VARCHAR(20) DEFAULT 'comment' CHECK (comment_type IN ('comment', 'status_update', 'assignment', 'system')),
...
-- Client Onboarding Steps Table (Alternative/Additional)
...
-- Step Documents Table (Alternative)
...
-- Step Comments Table (Alternative)
...
-- Template indexes
CREATE INDEX IF NOT EXISTS idx_templates_category_id ON onboarding_templates(category_id);
CREATE INDEX IF NOT EXISTS idx_templates_template_type_id ON onboarding_templates(template_type_id);
CREATE INDEX IF NOT EXISTS idx_templates_is_active ON onboarding_templates(is_active);
...
-- Function to calculate next run time for FinOps tasks
...
-- Function to update project progress based on step completion
...
-- Function to generate ticket track ID
CREATE OR REPLACE FUNCTION generate_ticket_track_id()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.track_id IS NULL OR NEW.track_id = '' THEN
        NEW.track_id := 'TKT-' || LPAD(nextval('tickets_id_seq')::text, 4, '0');
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
-- Apply triggers to relevant tables for updated_at timestamp
...
DROP TRIGGER IF EXISTS update_client_onboarding_steps_updated_at ON client_onboarding_steps;
CREATE TRIGGER update_client_onboarding_steps_updated_at BEFORE UPDATE ON client_onboarding_steps FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
-- FinOps triggers
DROP TRIGGER IF EXISTS trigger_update_next_run ON finops_tasks;
...
DROP TRIGGER IF EXISTS trigger_update_subtask_due ON finops_subtasks;
...
-- Workflow project progress triggers
...
-- Ticket track ID generation trigger
DROP TRIGGER IF EXISTS trigger_generate_ticket_track_id ON tickets;
CREATE TRIGGER trigger_generate_ticket_track_id
    BEFORE INSERT ON tickets
    FOR EACH ROW EXECUTE FUNCTION generate_ticket_track_id();

-- =====================================================
-- VIEWS FOR REPORTING
-- =====================================================

-- FinOps Task Dashboard View
...
INSERT INTO template_categories (name, description, color, icon, sort_order) VALUES 
    ...
    ('Support', 'Customer support and service templates', '#EF4444', 'Headphones', 5),
...
((SELECT id FROM onboarding_templates WHERE name = 'Standard Client Onboarding' LIMIT 1), 1, 'Initial Contact', 'Reach out to the client to introduce the onboarding process.', 2, true, true),
((SELECT id FROM onboarding_templates WHERE name = 'Standard Client Onboarding' LIMIT 1), 2, 'Document Collection', 'Gather all necessary legal and financial documents from the client.', 5, true, true),
((SELECT id FROM onboarding_templates WHERE name = 'Standard Client Onboarding' LIMIT 1), 3, 'Contract Signing', 'Review and execute service agreements.', 3, true, false),
((SELECT id FROM onboarding_templates WHERE name = 'Standard Client Onboarding' LIMIT 1), 4, 'Account Setup', 'Create client accounts and configure initial settings.', 2, false, true),
((SELECT id FROM onboarding_templates WHERE name = 'Standard Client Onboarding' LIMIT 1), 5, 'Training Session', 'Conduct onboarding training and knowledge transfer.', 7, false, true)
ON CONFLICT DO NOTHING;
...
('Waiting for Customer', '#8B5CF6', false, 3),
...
('Bug Report', 'Software bugs and issues', '#EF4444'),
('Feature Request', 'New feature requests', '#10B981'),
('Support', 'General support inquiries', '#3B82F6'),
('Technical Issue', 'Technical problems and questions', '#F59E0B')
ON CONFLICT (name) DO NOTHING;
...
-- This completes the unified database schema for the Banani application
-- All tables, indexes, functions, triggers, and default data are included
