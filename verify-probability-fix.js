...
async function verifyProbabilityFix() {
  ...
  try {
    ...
    for (const template of templates.rows) {
      ...
      console.log(`Steps: ${template.step_count}, Total: ${template.total_prob}%`);
    ...
    try {
      ...
      if (response.ok) {
        ...
        const apiCalculated = totalApiProb > 0 ? Math.round((completedApiProb / totalApiProb) * 100) : 0;
        ...
        const leadData = await pool.query("SELECT id, title, probability FROM leads WHERE id = 1");
        if (leadData.rows.length > 0) {
          ...
          console.log(`Match: ${lead.probability === apiCalculated ? "✅ CORRECT" : "❌ MISMATCH"}`);
          
          if (lead.probability !== apiCalculated) {
            console.log(`🔧 Updating database to match calculation...`);
            await pool.query("UPDATE leads SET probability = $1 WHERE id = $2", [apiCalculated, lead.id]);
    ...
    if (testSteps.rows.length > 0) {
      ...
      console.log(`Testing step: "${testStep.name}" (${testStep.probability_percent}%)`);
      ...
      allSteps.rows.forEach(step => {
        ...
        const stepStatus = step.status === testStep.status && step.probability_percent === testStep.probability_percent ? newStatus : step.status;
      ...
      const expectedProb = totalProb > 0 ? Math.round((completedProb / totalProb) * 100) : 0;
    ...
    console.log("4. Progress should use template percentages, not equal distribution");
