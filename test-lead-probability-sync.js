...
async function testLeadProbabilitySync() {
  try {
    ...
    for (const leadId of testLeads) {
      ...
      try {
        // Get lead steps
        const response = await fetch(`http://localhost:8080/api/leads/${leadId}/steps`);
        ...
        if (totalProbability === 0) {
          console.log("⚠️  WARNING: All probabilities are 0% - sync might not be working");
        } else if (totalProbability === 100) {
          console.log("✅ Perfect! Probabilities total exactly 100%");
        } else {
          console.log(`📊 Probabilities total ${totalProbability}% (may be partial completion)`);
        }
    ...
    console.log("\n" + "=".repeat(60));
    console.log("📋 SUMMARY:");
    console.log("- If lead steps show 0% probabilities, run: node sync-lead-step-probabilities.js");
    console.log("- If template shows 0% probabilities, run: node fix-template-probability.js");
    console.log("- For complete workflow fix, run: node complete-workflow-fix.js");
