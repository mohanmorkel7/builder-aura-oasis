...
async function runDiagnostics() {
  ...
  const finopsTest = await testEndpoint('/finops/test', 'FinOps API test endpoint');
  ...
  const debugTest = await testEndpoint('/finops/debug/status', 'FinOps debug status');
  const tasksTest = await testEndpoint('/finops/tasks', 'FinOps tasks (main endpoint)');
  const activityTest = await testEndpoint('/finops/activity-log', 'FinOps activity log');
  ...
  const productionTasksTest = await testEndpoint('/finops-production/tasks', 'FinOps production tasks');
  const productionHealthTest = await testEndpoint('/finops-production/health', 'FinOps production health');
  ...
  if (!serverTest.success) {
    ...
    if (tasksTest.networkError) {
      console.log('🔧 Check if FinOps routes are properly loaded in server/index.ts');
    } else {
      console.log('🔧 Check database connection and FinOps table schema');
    }
  } else {
    console.log('✅ FinOps API appears to be working correctly');
    console.log('🔧 If frontend issues persist, check for FullStory interference');
