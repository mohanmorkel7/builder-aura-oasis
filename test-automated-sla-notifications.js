...
async function testAutomatedSLA() {
  ...
  const testSubtasks = await makeRequest('/test/create-timed-subtasks', { method: 'POST' });
  if (testSubtasks.success) {
    ...
    testSubtasks.data.subtasks.forEach(subtask => {
      console.log(`  - ${subtask.name}: ${subtask.start_time} (${subtask.description})`);
    });
  } else {
  ...
  if (autoSync.success) {
    console.log(`✅ Auto-sync complete: ${autoSync.data.notifications_created} notifications created`);
  ...
  if (notifications.success) {
    console.log(`✅ Retrieved ${notifications.data.notifications.length} notifications from database`);
