...
async function testVCChatEdit() {
  ...
  try {
    ...
    const hasStepId = columnCheck.rows.some(row => row.column_name === 'step_id');
    const hasMessageType = columnCheck.rows.some(row => row.column_name === 'message_type');
    const hasIsRichText = columnCheck.rows.some(row => row.column_name === 'is_rich_text');
    ...
    `, [
      createdComment.id,
      "Updated test message for VC step chat",
      true
    ]);
    ...
    console.log(`✅ Found ${findResult.rows.length} comments for step ${stepId}`);
