  ...
  try {
    ...
    if (response.ok && result.success) {
      ...
      if (result.files && result.files.length > 0) {
        ...
        const downloadResponse = await fetch(`http://localhost:5173/api/files/download/${uploadedFile.filename}`);
        if (downloadResponse.ok) {
          console.log("✅ Download test successful!");
          const fileContent = await downloadResponse.text();
          console.log("📄 First 100 characters of downloaded file:", fileContent.substring(0, 100));
        } else {
