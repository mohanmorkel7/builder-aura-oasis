...
async function testVCChatAPI() {
  ...
  try {
    ...
    const createResponse = await axios.post(`${API_BASE}/vc/steps/1/chats`, createChatData);
    ...
    try {
      const editResponse = await axios.put(`${API_BASE}/vc/chats/${chatId}`, editData);
      console.log("✅ Successfully edited VC chat");
      console.log("Updated message:", editResponse.data.message);
    } catch (editError) {
      console.error("❌ Error editing VC chat:", editError.response?.data || editError.message);
      ...
      try {
        ...
        const leadsEditResponse = await axios.put(`${API_BASE}/leads/chats/${chatId}`, editData);
        console.log("❌ Leads endpoint worked - this is the problem! It should use VC endpoint.");
      } catch (leadsError) {
    ...
    try {
      const deleteResponse = await axios.delete(`${API_BASE}/vc/chats/${chatId}`);
      console.log("✅ Successfully deleted VC chat");
    } catch (deleteError) {
      console.error("❌ Error deleting VC chat:", deleteError.response?.data || deleteError.message);
    }
    
    console.log("\n✅ All VC chat API tests completed!");
    
  } catch (error) {
    console.error("❌ Error testing VC chat API:", error.response?.data || error.message);
    
    if (error.code === 'ECONNREFUSED') {
      console.log("💡 Make sure the development server is running on port 8080");
    } else if (error.response?.status === 404) {
      console.log("💡 The endpoint might not exist or the VC/step ID is invalid");
    }
  }
}
...
async function testWithMockData() {
  ...
  try {
    ...
    const mockCreateResponse = await axios.post(`${API_BASE}/vc/steps/2/chats`, mockCreateData);
    ...
    const mockEditResponse = await axios.put(`${API_BASE}/vc/chats/${mockCreateResponse.data.id}`, mockEditData);
    console.log("✅ Successfully edited mock VC chat");
    
  } catch (error) {
    console.error("❌ Error with mock data:", error.response?.data || error.message);
  }
}
