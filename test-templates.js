...
async function testTemplateEndpoints() {
  ...
  try {
    ...
    console.log(`Found ${allTemplatesData.length} templates:`, allTemplatesData.map(t => ({ id: t.id, name: t.name, category_id: t.category_id })));

    // Test 2: Get template categories
    console.log('\n2. Testing GET /templates-production/categories:');
    const categories = await fetch(`${API_BASE}/templates-production/categories`);
    const categoriesData = await categories.json();
    console.log(`Found ${categoriesData.length} categories:`, categoriesData.map(c => ({ id: c.id, name: c.name })));

    // Test 3: Get templates by category (category ID 2 = Leads)
    console.log('\n3. Testing GET /templates-production/category/2:');
    const categoryTemplates = await fetch(`${API_BASE}/templates-production/category/2`);
    const categoryTemplatesData = await categoryTemplates.json();
    console.log(`Found ${categoryTemplatesData.length} templates for category 2 (Leads):`, categoryTemplatesData.map(t => ({ id: t.id, name: t.name, category_id: t.category_id })));
    ...
    if (createResponse.ok) {
      ...
      console.log('Created template:', { id: createdTemplate.id, name: createdTemplate.name, category_id: createdTemplate.category_id });
      ...
      const updatedCategoryTemplates = await fetch(`${API_BASE}/templates-production/category/2`);
      const updatedCategoryTemplatesData = await updatedCategoryTemplates.json();
      console.log(`Found ${updatedCategoryTemplatesData.length} templates for category 2 (after creation):`, updatedCategoryTemplatesData.map(t => ({ id: t.id, name: t.name, category_id: t.category_id })));
    } else {
