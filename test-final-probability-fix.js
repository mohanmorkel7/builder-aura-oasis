  ...
  try {
    ...
    if (totalTemplateProb !== 100 && template1Steps.rows.length > 0) {
      ...
      if (template1Steps.rows.length === 2) {
        ...
        await pool.query('UPDATE template_steps SET probability_percent = 30 WHERE id = $1', [template1Steps.rows[0].id]);
        await pool.query('UPDATE template_steps SET probability_percent = 70 WHERE id = $1', [template1Steps.rows[1].id]);
        ...
        for (let i = 0; i < template1Steps.rows.length; i++) {
          ...
          await pool.query('UPDATE template_steps SET probability_percent = $1 WHERE id = $2', 
            [probability, template1Steps.rows[i].id]);
          console.log(`  ✅ Set ${template1Steps.rows[i].name} = ${probability}%`);
        }
      }
    }
    ...
    for (const lead of leadsWithTemplate1.rows) {
      ...
      console.log(`  Lead has ${leadSteps.rows.length} steps, template has ${updatedTemplateSteps.rows.length} steps`);

      // Sync probabilities
      for (const leadStep of leadSteps.rows) {
        const matchingTemplate = updatedTemplateSteps.rows.find(ts => 
          ts.step_order === leadStep.step_order && 
          ts.name.toLowerCase().trim() === leadStep.name.toLowerCase().trim()
        );

        if (matchingTemplate && (leadStep.probability_percent || 0) !== matchingTemplate.probability_percent) {
          await pool.query(`
            UPDATE lead_steps 
          ...
          console.log(`    📊 Updated "${leadStep.name}": ${leadStep.probability_percent || 0}% → ${matchingTemplate.probability_percent}%`);
        }
      }
    }
    ...
    // Test with lead 1
    const testLead = await pool.query('SELECT id, title, probability FROM leads WHERE id = 1');
    if (testLead.rows.length > 0) {
      ...
      steps.rows.forEach((step, idx) => {
        ...
        console.log(`  ${idx + 1}. ${step.name}: ${stepProbability}% (${step.status}) → contributes ${contribution}%`);
      });

      const calculatedPercentage = totalStepProbability > 0 
      ...
      console.log(`  Total Completed Probability: ${totalCompletedProbability}%`);
      console.log(`  Calculated Progress: ${calculatedPercentage}%`);

      // Update the lead probability in database
      await pool.query('UPDATE leads SET probability = $1 WHERE id = $2', [calculatedPercentage, lead.id]);
      ...
      if (steps.rows.length >= 1) {
        ...
        await pool.query('UPDATE lead_steps SET status = $1 WHERE id = $2', ['completed', firstStep.id]);
        ...
        // Reset status
        await pool.query('UPDATE lead_steps SET status = $1 WHERE id = $2', [firstStep.status, firstStep.id]);
      }
    }
    ...
    try {
      ...
      // Test steps API
      const stepsResponse = await fetch('http://localhost:8080/api/leads/1/steps');
      if (stepsResponse.ok) {
        const stepsData = await stepsResponse.json();
        console.log(`✅ Steps API: ${stepsData.length} steps returned`);
        
        stepsData.forEach((step, idx) => {
          console.log(`  ${idx + 1}. ${step.name}: ${step.probability_percent || 0}% (${step.status})`);
        });

        const apiTotal = stepsData.reduce((sum, step) => sum + (step.probability_percent || 0), 0);
    ...
    console.log('2. Change a step status and verify progress updates correctly');
    console.log('3. Check that progress uses template probability values, not 50-50 split');
