...
async function setupSLATestData() {
  try {
    ...
    const warningTimeStr = warningTime.toTimeString().split(' ')[0].substring(0, 5); // HH:MM format
    
    console.log('⏰ Setting up subtask to start at:', warningTimeStr, '(should trigger warning in ~10 min)');
    ...
    for (let i = 0; i < subtasks.length; i++) {
      ...
      console.log(`✅ Created subtask: ${subtask.name} (${subtask.start_time})`);
    }
    
    // Test the SLA function
    console.log('🔍 Testing SLA function...');
    const slaResult = await pool.query('SELECT * FROM check_subtask_sla_notifications()');
    ...
    if (slaResult.rows.length === 0) {
      ...
      console.log('   - The current time logic in the function needs adjustment');
    } else {
