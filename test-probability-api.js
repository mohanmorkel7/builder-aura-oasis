  ...
  try {
    ...
    if (data.length > 0) {
      ...
      data.forEach((step, index) => {
        console.log(`${index + 1}. ${step.name}: ${step.probability_percent || 0}%`);
      });
      
      const totalProbability = data.reduce((sum, step) => sum + (step.probability_percent || 0), 0);
      console.log(`\n🎯 Total probability: ${totalProbability}%`);
      
      if (totalProbability === 0) {
        console.log("⚠️  WARNING: All probability_percent values are 0 - this suggests:");
