...
export class WorkflowRepository {
  ...
  static async getCompletedLeads(): Promise<any[]> {
    const result = await pool.query(
      `SELECT
         l.id,
         l.client_name,
         ...
         l.status,
         l.created_at as completion_date,
         l.project_value as estimated_budget,
         COALESCE(ls_stats.total_steps, 0) as total_steps,
         COALESCE(ls_stats.completed_steps, 0) as completed_steps,
         'ready_for_product' as product_status
       FROM leads l
       LEFT JOIN (
         SELECT
           lead_id,
