  ...
  steps: [
    {
      ...
      step_description: "Create the foundational architecture using our existing platform components",
      step_order: 1,
      status: "in_progress",
      assigned_to: 3,
    ...
    },
    {
      ...
      step_description: "Coordinate with development team and assign specific tasks with tracking",
      step_order: 2,
      status: "pending",
      assigned_to: 2,
    ...
    },
    {
      ...
      step_description: "Run automated transaction reconciliation at 5:00 AM daily",
      step_order: 1,
      status: "completed",
      is_automated: true,
      automation_config: {
    ...
    },
    {
      ...
      step_description: "Ensure all files are processed before the 5 AM cutoff time",
      step_order: 2,
      status: "in_progress",
      is_automated: true,
      automation_config: {
  ...
  comments: [
    {
      ...
      project_id: 1,
      step_id: 1,
      comment_text: "Started working on the base platform setup. Using React and Node.js stack as discussed.",
    ...
    },
    {
      id: 2,
      project_id: 1,
      comment_text: "Project milestone reached - 45% completion. Moving to next phase.",
  ...
  notifications: [
    {
      ...
      message: "The step 'Follow-up with development team' is overdue. Please take action.",
      recipient_id: 2,
      project_id: 1,
      step_id: 2,
      source_type: "product",
    ...
    },
    {
      ...
      message: "The automated reconciliation process failed at 5:00 AM. Manual intervention required.",
      recipient_id: 4,
      project_id: 2,
      source_type: "finops",
      priority: "critical",
...
router.get("/dashboard", async (req: Request, res: Response) => {
  try {
    ...
    if (await isDatabaseAvailable()) {
      const dashboardData = await WorkflowRepository.getDashboardData(userId, userRole);
...
router.post("/projects", async (req: Request, res: Response) => {
  try {
    ...
    if (!projectData.name || !projectData.project_type || !projectData.created_by) {
      return res.status(400).json({ error: "Missing required fields: name, project_type, created_by" });
    }
...
router.post("/projects/from-lead/:leadId", async (req: Request, res: Response) => {
  try {
...
// Get project steps
router.get("/projects/:projectId/steps", async (req: Request, res: Response) => {
  try {
    ...
    if (await isDatabaseAvailable()) {
      ...
      const mockSteps = WorkflowMockData.steps.filter(s => s.project_id === projectId);
      res.json(mockSteps);
    }
  } catch (error) {
...
// Create step
router.post("/projects/:projectId/steps", async (req: Request, res: Response) => {
  try {
    const projectId = parseInt(req.params.projectId);
    if (isNaN(projectId)) {
      return res.status(400).json({ error: "Invalid project ID" });
    ...
    if (!stepData.step_name || !stepData.created_by) {
      return res.status(400).json({ error: "Missing required fields: step_name, created_by" });
    }
...
router.patch("/steps/:stepId/status", async (req: Request, res: Response) => {
  try {
    ...
    if (!status || !updated_by) {
      return res.status(400).json({ error: "Missing required fields: status, updated_by" });
    }
...
router.get("/projects/:projectId/comments", async (req: Request, res: Response) => {
  try {
    const projectId = parseInt(req.params.projectId);
    const stepId = req.query.stepId ? parseInt(req.query.stepId as string) : undefined;
    ...
    if (await isDatabaseAvailable()) {
      const comments = await WorkflowRepository.getProjectComments(projectId, stepId);
      res.json(comments);
    } else {
      let mockComments = WorkflowMockData.comments.filter(c => c.project_id === projectId);
      if (stepId) {
...
// Create comment
router.post("/projects/:projectId/comments", async (req: Request, res: Response) => {
  try {
    const projectId = parseInt(req.params.projectId);
    if (isNaN(projectId)) {
      return res.status(400).json({ error: "Invalid project ID" });
    ...
    if (!commentData.comment_text || !commentData.created_by) {
      return res.status(400).json({ error: "Missing required fields: comment_text, created_by" });
    }
...
router.get("/notifications", async (req: Request, res: Response) => {
  try {
    ...
    if (await isDatabaseAvailable()) {
      const notifications = await WorkflowRepository.getUserNotifications(userId, unreadOnly);
      res.json(notifications);
    } else {
      let mockNotifications = WorkflowMockData.notifications.filter(n => n.recipient_id === userId);
      if (unreadOnly) {
      ...
      const mockAutomations = [
        {
          ...
          schedule_config: { time: "05:00", days: ["monday", "tuesday", "wednesday", "thursday", "friday"] },
          is_active: true,
        ...
        },
        {
          ...
          schedule_config: { time: "04:45", days: ["monday", "tuesday", "wednesday", "thursday", "friday"] },
          is_active: true,
...
router.patch("/projects/:projectId/status", async (req: Request, res: Response) => {
  try {
    ...
    if (await isDatabaseAvailable()) {
      await WorkflowRepository.updateProjectStatus(projectId, status);
      res.json({ success: true, message: "Project status updated" });
    } else {
      // Update mock data for testing
      const mockProject = WorkflowMockData.projects.find(p => p.id === projectId);
      if (mockProject) {
