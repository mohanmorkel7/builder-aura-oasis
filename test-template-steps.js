  ...
  try {
    ...
    if (data.steps && data.steps.length > 0) {
      ...
      if (totalProbability === 0) {
        ...
        console.log("   - Database column 'probability_percent' might not exist");
        console.log("   - Mock data is being used without probability_percent");
        console.log("   - Template steps query doesn't include probability_percent field");
      } else if (totalProbability === 100) {
        console.log("✅ Perfect! Template totals exactly 100%");
      } else {
        console.log(`⚠️  Warning: Template total is ${totalProbability}%, not 100%`);
      }
    } else {
