...
const mockApiCalls = {
  ...
  performDeploymentAction: async (action: DeploymentAction) => {
    console.log(`Performing action: ${action.type} on deployment ${action.deploymentId}`);
  ...
  getDeploymentProgress: async (deploymentId: string): Promise<DeploymentProgress> => {
    const mockProgress: DeploymentProgress = {
      ...
      steps: [
        { name: 'Build Application', status: 'completed', duration: '2m 15s', startTime: '10:30:15', endTime: '10:32:30' },
        { name: 'Run Tests', status: 'completed', duration: '5m 30s', startTime: '10:32:30', endTime: '10:38:00' },
        { name: 'Deploy to Staging', status: 'completed', duration: '1m 45s', startTime: '10:38:00', endTime: '10:39:45' },
        { name: 'Run Integration Tests', status: 'in_progress', startTime: '10:39:45' },
...
export function useDeploymentControl(deploymentId?: string) {
  ...
  const statusMutation = useMutation({
    mutationFn: ({ deploymentId, status }: { deploymentId: string; status: string }) =>
      mockApiCalls.updateDeploymentStatus(deploymentId, status),
    onSuccess: (data, variables) => {
      // Invalidate deployments query to refresh the list
      queryClient.invalidateQueries({ queryKey: ['deployments'] });
      console.log(`Status updated successfully for deployment ${variables.deploymentId}`);
    },
    onError: (error) => {
  ...
  const actionMutation = useMutation({
    mutationFn: (action: DeploymentAction) => mockApiCalls.performDeploymentAction(action),
    onSuccess: (data, variables) => {
      queryClient.invalidateQueries({ queryKey: ['deployments'] });
  ...
  const pollProgress = useCallback(async () => {
    ...
    try {
      const progressData = await mockApiCalls.getDeploymentProgress(deploymentId);
      setProgress(progressData);

      // Stop polling if deployment is complete or failed
      if (['completed', 'failed', 'stopped', 'paused'].includes(progressData.status)) {
        setIsPolling(false);
      }
    } catch (error) {
...
export function useDeploymentStatus(deploymentId: string) {
  ...
  const { progress, isPolling, startPolling } = useDeploymentControl(deploymentId);
