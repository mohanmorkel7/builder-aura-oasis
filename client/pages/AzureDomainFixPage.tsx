...
export default function AzureDomainFixPage() {
  ...
  const currentDomain = 'd7b2d3c602404b4b86d25bbaa919fce3-ff6713a8d48143f18afd4ef92.fly.dev';
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      <div className="text-center">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Azure AD Domain Configuration Fix</h1>
        ...
        <CardContent className="space-y-3">
          <div className="p-3 bg-red-100 rounded-lg">
            <p className="text-red-800 text-sm font-medium mb-2">
              ❌ <strong>Root Cause Found:</strong> Azure AD doesn't recognize your Fly.dev domain
            </p>
            <p className="text-red-700 text-sm">
              Even with admin consent granted, Azure AD rejects authentication requests from unauthorized domains for security reasons.
            </p>
          </div>
          
          <div className="p-3 bg-yellow-100 rounded-lg">
            <p className="text-yellow-800 text-sm">
              <strong>Your Current Domain:</strong> <code className="bg-yellow-200 px-1 rounded">{currentDomain}</code>
            </p>
        ...
        <CardContent className="space-y-4">
          <div className="p-4 bg-blue-50 rounded-lg">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-blue-800">Current Application URL</p>
              ...
              >
                {copied === 'domain' ? <CheckCircle className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
              </Button>
            </div>
          </div>
        </CardContent>
        ...
        <CardContent className="space-y-4">
          <div className="p-4 bg-orange-50 border border-orange-200 rounded-lg">
            ...
            <p className="text-orange-700 text-sm mb-3">
              Copy each URL below and add them to Azure AD → Authentication → Redirect URIs:
            </p>
            
            <div className="space-y-2">
              {redirectURIs.map((uri, index) => (
                <div key={index} className="flex items-center space-x-2 p-3 bg-white border rounded">
                  ...
                  >
                    {copied === `uri-${index}` ? <CheckCircle className="w-4 h-4 text-green-500" /> : <Copy className="w-4 h-4" />}
                  </Button>
                </div>
              ))}
            </div>
          ...
          <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
            ...
            <div className="space-y-2 text-sm text-green-700">
              <div className="flex items-center space-x-2">
                ...
                <span>Access tokens (used for implicit flows): <strong>Enabled</strong></span>
              </div>
              <div className="flex items-center space-x-2">
                <CheckCircle className="w-4 h-4" />
                <span>ID tokens (used for implicit and hybrid flows): <strong>Enabled</strong></span>
              </div>
              <div className="flex items-center space-x-2">
                <CheckCircle className="w-4 h-4" />
                <span>Allow public client flows: <strong>Yes</strong></span>
              </div>
            </div>
          </div>
        </CardContent>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            <div className="p-4 bg-blue-50 rounded-lg">
              <h4 className="font-medium text-blue-800 mb-2">🔧 Azure Portal Steps</h4>
              <ol className="list-decimal list-inside space-y-2 text-sm text-blue-700">
                <li>Click "Open Azure AD Authentication" below</li>
                <li>Scroll to "Platform configurations" section</li>
                <li>If you see existing "Web" platforms, click "Configure" on one of them</li>
                <li>If no Web platforms exist, click "Add a platform" → "Web"</li>
                <li>Add ALL the redirect URIs listed above (copy/paste each one)</li>
            ...
            <div className="p-4 bg-purple-50 rounded-lg">
              <h4 className="font-medium text-purple-800 mb-2">🧪 Test After Configuration</h4>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <Button 
              onClick={() => window.open(azureConfig.authenticationUrl, '_blank')}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            <div className="p-3 bg-gray-50 rounded">
              ...
              <div className="flex items-center space-x-2">
                ...
                >
                  {copied === 'clientId' ? <CheckCircle className="w-3 h-3" /> : <Copy className="w-3 h-3" />}
                </Button>
              </div>
            </div>
            ...
            <div className="p-3 bg-gray-50 rounded">
              ...
              <div className="flex items-center space-x-2">
                ...
                >
                  {copied === 'tenantId' ? <CheckCircle className="w-3 h-3" /> : <Copy className="w-3 h-3" />}
                </Button>
              </div>
            </div>
          </div>
