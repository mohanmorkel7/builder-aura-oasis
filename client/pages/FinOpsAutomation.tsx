...
export default function FinOpsAutomation() {
  ...
  const { data: finopsProjects = [], isLoading: projectsLoading } = useQuery({
    ...
    queryFn: () => apiClient.getWorkflowProjects(parseInt(user?.id || "1"), "finance"),
  });
  ...
  const { data: notifications = [], isLoading: notificationsLoading } = useQuery({
    ...
    queryFn: () => apiClient.getWorkflowNotifications(parseInt(user?.id || "1"), true),
  });

  const triggerAutomationMutation = useMutation({
    mutationFn: (automationId: number) => apiClient.triggerAutomation(automationId),
    onSuccess: () => {
  ...
  const dailyProcessSteps: ProcessStep[] = [
    ...
    },
    {
      ...
      step_description: "Coordinate with FinOps team members on daily tasks and any issues",
      status: "in_progress",
    ...
    },
    {
      ...
      step_description: "Send alerts to lead and FinOps teams if any processes fail or are delayed",
      status: "pending",
      is_automated: true,
      automation_config: {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <Tabs defaultValue="daily-processes" className="space-y-6">
                        ...
                        <div className="flex items-start justify-between">
                          <div>
                            ...
                            <div className="flex items-center gap-4 mt-2 text-xs text-gray-500">
                              {step.start_time && (
                                <span>Started: {format(new Date(step.start_time), "h:mm a")}</span>
                              )}
                              {step.completion_time && (
                                <span>Completed: {format(new Date(step.completion_time), "h:mm a")}</span>
                              )}
                              {step.is_automated && (
                          ...
                          <div className="flex gap-2">
                            ...
                            {step.status === "in_progress" && !step.is_automated && (
                              <Button size="sm">
                      ...
                      <div className="flex items-start gap-4">
                        <div className="flex items-center gap-2">
                          <Switch 
                            ...
                            onChange={() => {}} 
                          />
                          <Timer className={`w-4 h-4 ${getAutomationStatusColor(automation)}`} />
                        </div>
                        
                        <div>
                          <h3 className="font-medium">{automation.automation_name}</h3>
                          ...
                          <div className="flex items-center gap-4 mt-2 text-xs text-gray-500">
                            {automation.last_run_at && (
                              <span>Last run: {format(new Date(automation.last_run_at), "MMM d, h:mm a")}</span>
                            )}
            ...
            <CardContent>
              {notificationsLoading ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-600">All automated processes are running smoothly.</p>
                </div>
              )}
            </CardContent>
          </Card>
        ...
        <TabsContent value="lead-integration" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                FinOps processes connected to leads and product development projects
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                <Alert>
                  ...
                  <AlertDescription>
                    FinOps processes automatically track financial data from completed leads and active product projects.
                    Daily reconciliation includes data from both lead conversions and project expenses.
                  </AlertDescription>
                </Alert>
                ...
                {finopsProjects.length > 0 ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <h3 className="text-lg font-medium text-gray-900 mb-2">No FinOps Projects</h3>
                    <p className="text-gray-600">FinOps processes will appear here when leads are converted to projects.</p>
                  </div>
                )}
              </div>
            </CardContent>
