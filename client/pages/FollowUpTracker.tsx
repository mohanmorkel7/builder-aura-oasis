...
const mockFollowUps: FollowUp[] = [
  {
    ...
    original_message: "Client requirements documented and shared with the team. @Mike Johnson please review the technical specifications #13",
  ...
  },
  {
    ...
    original_message: "Working on the proposal. Need technical specifications from the development team. Follow up needed on API documentation #14",
  ...
  },
  {
    ...
    original_message: "Demo feedback received. Client wants additional reporting features. @Product Team please assess timeline impact #15",
    assigned_to: "Mike Johnson",
  ...
  },
  {
    ...
    step_name: "Initial Contact", 
    original_message: "Compliance requirements discussion scheduled. @Legal Team review banking regulations for data handling #16",
...
export default function FollowUpTracker() {
  ...
  const filteredFollowUps = mockFollowUps.filter((followUp) => {
    const matchesSearch =
      ...
      followUp.original_message.toLowerCase().includes(searchTerm.toLowerCase()) ||
      followUp.id.toString().includes(searchTerm);

    const matchesStatus = statusFilter === "all" || followUp.status === statusFilter;
    const matchesAssignee = assigneeFilter === "all" || followUp.assigned_to === assigneeFilter;

    return matchesSearch && matchesStatus && matchesAssignee;
  });

  const myFollowUps = filteredFollowUps.filter(f => f.assigned_to === user?.name);
  const assignedByMe = filteredFollowUps.filter(f => f.assigned_by === user?.name);
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-blue-900">
                  {mockFollowUps.filter(f => f.status === 'in_progress').length}
                </p>
              </div>
        ...
        {filteredFollowUps.length === 0 ? (
          ...
          filteredFollowUps.map((followUp) => {
            ...
            const isOverdue = followUp.status === 'overdue' || 
              (followUp.status !== 'completed' && new Date(followUp.due_date) < new Date());
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center space-x-4 text-sm text-gray-600 mb-3">
                        <span className="flex items-center space-x-1">
                          ...
                          <span>Assigned to: <strong>{followUp.assigned_to}</strong></span>
                        </span>
                        <span className="flex items-center space-x-1">
                          <User className="w-4 h-4" />
                      ...
                      <div className="flex items-center space-x-4 text-xs text-gray-500">
                        <span>
                          Created: {new Date(followUp.created_at).toLocaleDateString("en-IN", {
                            timeZone: "Asia/Kolkata",
                          })}
                        </span>
                        {followUp.completed_at && (
                          <span>
                            Completed: {new Date(followUp.completed_at).toLocaleDateString("en-IN", {
                              timeZone: "Asia/Kolkata",
                            })}
                          </span>
                        )}
                    ...
                    <div className="flex flex-col space-y-2">
                      ...
                      {followUp.status !== 'completed' && (
                        <Select
                          ...
                          onValueChange={(value) => handleUpdateStatus(followUp.id, value)}
                        >
