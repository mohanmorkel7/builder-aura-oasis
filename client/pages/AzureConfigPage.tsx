  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      <div className="text-center">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Azure AD Configuration Checker</h1>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="p-3 bg-blue-50 rounded-lg">
            <p className="text-sm"><strong>Current Domain:</strong></p>
            <p className="font-mono text-blue-800">{currentDomain}</p>
          </div>
          
          <div className="p-3 bg-green-50 rounded-lg">
            <p className="text-sm"><strong>Deployment Platform:</strong></p>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
            ...
            <div className="p-3 bg-gray-50 rounded-lg">
              ...
              <div className="flex items-center space-x-2">
                ...
                <Button
                  size="sm"
                  variant="ghost"
                  onClick={() => copyToClipboard(azureConfig.clientId, 'clientId')}
                >
            ...
            <div className="p-3 bg-gray-50 rounded-lg">
              ...
              <div className="flex items-center space-x-2">
                ...
                <Button
                  size="sm"
                  variant="ghost"
                  onClick={() => copyToClipboard(azureConfig.tenantId, 'tenantId')}
                >
                  {copied === 'tenantId' ? <CheckCircle className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
                </Button>
              </div>
            </div>
          </div>
          ...
          <div className="flex space-x-3">
            <Button onClick={() => openAzurePortal('overview')} variant="outline">
              <ExternalLink className="w-4 h-4 mr-2" />
              Open Overview
            </Button>
            <Button onClick={() => openAzurePortal('authentication')} variant="outline">
              <Settings className="w-4 h-4 mr-2" />
              Open Authentication
            </Button>
            <Button onClick={() => openAzurePortal('permissions')} variant="outline">
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          {/* Redirect URIs */}
          <div>
            <h4 className="font-medium text-lg mb-3">1. Redirect URIs (Authentication)</h4>
            <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg mb-4">
              <p className="text-yellow-800 text-sm mb-2">
                <strong>⚠️ Critical:</strong> Your Fly.dev domain must be added to Azure AD redirect URIs
              </p>
            </div>
            
            <div className="space-y-2">
              <p className="text-sm font-medium">Add these redirect URIs in Azure Portal → Authentication → Platform configurations:</p>
              {azureConfig.requiredRedirectUris.map((uri, index) => (
                <div key={index} className="flex items-center space-x-2 p-2 bg-gray-50 rounded">
                  ...
                  >
                    {copied === `uri-${index}` ? <CheckCircle className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
                  </Button>
                </div>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              <div className="flex items-center space-x-2 p-2 bg-green-50 rounded">
                ...
                <span className="text-sm">User.Read.All (Admin consent required)</span>
                <Badge variant="outline" className="bg-green-100 text-green-800">Granted</Badge>
              </div>
              <div className="flex items-center space-x-2 p-2 bg-green-50 rounded">
                <CheckCircle className="w-4 h-4 text-green-500" />
                <span className="text-sm">Directory.Read.All (Admin consent required)</span>
                <Badge variant="outline" className="bg-green-100 text-green-800">Granted</Badge>
              </div>
              <div className="flex items-center space-x-2 p-2 bg-green-50 rounded">
                <CheckCircle className="w-4 h-4 text-green-500" />
                <span className="text-sm">openid, profile, email</span>
                <Badge variant="outline" className="bg-green-100 text-green-800">Standard</Badge>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="p-3 bg-blue-50 rounded-lg">
              <p className="text-sm text-blue-800 mb-2">Ensure these token settings are configured:</p>
              <ul className="list-disc list-inside space-y-1 text-sm text-blue-700">
                <li>Access tokens (used for implicit flows): ✅ Enabled</li>
                <li>ID tokens (used for implicit and hybrid flows): ✅ Enabled</li>
        ...
        <CardContent>
          <div className="space-y-4">
            <div className="p-4 bg-blue-50 rounded-lg">
              <h4 className="font-medium text-blue-800 mb-2">🔧 Step 1: Update Redirect URIs</h4>
            ...
            <div className="p-4 bg-green-50 rounded-lg">
              <h4 className="font-medium text-green-800 mb-2">✅ Step 2: Verify Permissions</h4>
              <ol className="list-decimal list-inside space-y-1 text-sm text-green-700">
                <li>Click "Open Permissions" button above</li>
                <li>Confirm all permissions show "Granted for [Your Org]"</li>
                <li>If any show "Not granted", click "Grant admin consent" again</li>
              </ol>
            </div>

            <div className="p-4 bg-purple-50 rounded-lg">
              <h4 className="font-medium text-purple-800 mb-2">🧪 Step 3: Test SSO</h4>
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex flex-wrap gap-3">
            <Button onClick={() => openAzurePortal('authentication')} className="bg-blue-600 hover:bg-blue-700">
              <Settings className="w-4 h-4 mr-2" />
              Fix Redirect URIs
            </Button>
            
            <Button onClick={() => openAzurePortal('permissions')} className="bg-green-600 hover:bg-green-700">
              <Key className="w-4 h-4 mr-2" />
              Verify Permissions
            </Button>
