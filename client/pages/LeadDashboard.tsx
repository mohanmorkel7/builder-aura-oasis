      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {(() => {
          ...
          const currentDueFollowUps = followUpsData.filter((followUp: any) => {
            ...
            return diffDays >= 0 && diffDays <= 7 && followUp.status !== 'completed'; // Due within 7 days
          });
                  ...
                  {followUpsLoading ? (
                    ...
                    <div className="max-h-64 overflow-y-auto">
                      {currentDueFollowUps.map((followUp: any, index: number) => {
                        ...
                        return (
                          <div key={followUp.id} className={`p-4 border-b border-gray-100 hover:bg-gray-50 transition-colors ${index === currentDueFollowUps.length - 1 ? 'border-b-0' : ''}`}>
                              ...
                              <div className="flex-1">
                                <div className="flex items-center space-x-2 mb-1">
                                  ...
                                  <Badge
                                    variant="outline"
                                    className={`text-xs ${
                                      isToday ? 'border-orange-300 text-orange-700 bg-orange-50' :
                                      isTomorrow ? 'border-yellow-300 text-yellow-700 bg-yellow-50' :
                                ...
                                <div className="flex items-center space-x-4 text-xs text-gray-500">
                                  <span>Lead: {followUp.lead_name || 'Unknown'}</span>
                                  <span>Step: {followUp.step_name || 'N/A'}</span>
                                  <span>Assigned to: {followUp.assigned_to_name || 'Unassigned'}</span>
                                </div>
                              </div>
                  ...
                  {followUpsLoading ? (
                    ...
                    <div className="max-h-64 overflow-y-auto">
                      {overdueFollowUps.map((followUp: any, index: number) => {
                        ...
                        return (
                          <div key={followUp.id} className={`p-4 border-b border-gray-100 hover:bg-red-50 transition-colors ${index === overdueFollowUps.length - 1 ? 'border-b-0' : ''}`}>
                            <div className="flex items-start justify-between">
                              <div className="flex-1">
                                <div className="flex items-center space-x-2 mb-1">
                                  <h4 className="font-medium text-gray-900 text-sm line-clamp-1">
                              ...
                              <div className="text-right text-xs text-red-600 font-medium">
                                Due: {formatToIST(followUp.due_date).split(' ')[0]}
                              </div>
                            </div>
                          </div>
                        );
