...
import { Users, Database, CheckCircle, AlertTriangle, Loader } from 'lucide-react';
  ...
  const loadDepartments = async () => {
    try {
      ...
      if (result.success && result.data) {
        ...
        addLog(`✅ Department mapping loaded: ${Object.keys(result.data.departments).length} departments, ${result.data.users.length} users`);
        
        // Check if Mohan is in the mapping
        const mohanUser = result.data.users.find((u: any) => u.email === 'mohan.m@mylapay.com');
        if (mohanUser) {
          addLog(`✅ Mohan Raj found in mapping: department=${mohanUser.department}, jobTitle=${mohanUser.jobTitle}`);
        } else {
  ...
  const testSSOLogin = async () => {
    try {
      ...
      if (result.success) {
        ...
        addLog(`   Permissions: ${result.user.permissions?.join(', ') || 'None'}`);
      } else {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      <div className="text-center">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Department Mapping Test</h1>
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              <div className="flex items-center space-x-3">
                ...
                <Badge variant="default" className="bg-green-100 text-green-800">Mapping Loaded</Badge>
              </div>
              
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <strong>Departments:</strong> {Object.keys(departments.departments).length}
                ...
                <div>
                  ...
                  <ul className="mt-1 text-gray-600">
                    {departments.users.map((user: any) => (
                      <li key={user.email}>• {user.displayName} ({user.department})</li>
                    ))}
                  </ul>
                </div>
              </div>
            ...
            <div className={`p-3 rounded-lg ${testResult.success ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
              ...
              {testResult.success ? (
                <div className="mt-2 text-sm text-green-700">
                  <p><strong>Department:</strong> {testResult.user.department}</p>
                  <p><strong>Role:</strong> {testResult.user.role}</p>
                  <p><strong>Job Title:</strong> {testResult.user.jobTitle}</p>
                </div>
              ) : (
