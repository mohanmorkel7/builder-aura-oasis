...
import { formatToISTDateTime, getCurrentISTDateTime, IST_TIMEZONE } from '@/lib/dateUtils';
...
export function ISTTimeTest() {
  ...
  useEffect(() => {
    const updateTime = () => {
      ...
      setTestTimestamp(now.toLocaleString("en-IN", { 
  ...
  return (
    <div className="p-4 border rounded-lg bg-blue-50 mb-4">
      ...
      <div className="space-y-1 text-sm">
        <div><strong>Current IST (with indicator):</strong> {currentTime}</div>
        <div><strong>Raw IST format:</strong> {testTimestamp}</div>
        <div><strong>Browser timezone:</strong> {Intl.DateTimeFormat().resolvedOptions().timeZone}</div>
        <div><strong>IST Timezone:</strong> {IST_TIMEZONE}</div>
      </div>
    </div>
  );
}
