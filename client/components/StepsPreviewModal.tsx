...
export function StepsPreviewModal({
    ...
    const baseSteps = [
      {
        ...
        description: "Analyze and document detailed requirements from lead information",
        estimatedHours: 16,
        category: "Planning"
      },
      {
  ...
  const totalHours = projectSteps.reduce((sum, step) => sum + step.estimatedHours, 0);
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Estimated project steps based on lead requirements and budget analysis
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          <Card className="border-blue-200 bg-blue-50">
            <CardHeader>
              <CardTitle className="text-lg text-blue-800">Project Overview</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold text-blue-700">{projectSteps.length}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold text-blue-700">{Math.ceil(projectSteps.length / 3)}-{Math.ceil(projectSteps.length / 2)}</p>
                  </div>
                </div>
              </div>
            </CardContent>
          ...
          <Card>
            <CardHeader>
              ...
              <DialogDescription>
                These steps are automatically generated based on project scope and budget
              </DialogDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card className="border-green-200 bg-green-50">
            <CardHeader>
              <CardTitle className="text-lg text-green-800">Source Lead Information</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <p className="text-green-700">₹{lead.estimated_budget?.toLocaleString()}</p>
                </div>
                <div>
                  <Label className="font-medium">Lead Steps</Label>
                  <p className="text-green-700">{lead.completed_steps}/{lead.total_steps} completed</p>
                </div>
                <div>
                  <Label className="font-medium">Completion Date</Label>
                  <p className="text-green-700">{new Date(lead.completion_date).toLocaleDateString()}</p>
                </div>
              </div>
          ...
          <Card className="border-orange-200 bg-orange-50">
            <CardHeader>
              <CardTitle className="text-lg text-orange-800">Next Steps</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-2 text-sm">
                <div className="flex items-center gap-2 text-orange-700">
                  <ArrowRight className="w-4 h-4" />
                  <span>Click "Create Project" to convert this lead into a product development project</span>
                </div>
                <div className="flex items-center gap-2 text-orange-700">
                  <Settings className="w-4 h-4" />
                  <span>Steps can be customized and modified after project creation</span>
                </div>
                <div className="flex items-center gap-2 text-orange-700">
                  <Users className="w-4 h-4" />
                  <span>Team members will be assigned to individual steps during project setup</span>
                </div>
              </div>
            </CardContent>
          </Card>
