...
async function diagnoseDatabaseIssue() {
  try {
    ...
    const requiredTables = ['leads', 'template_steps', 'lead_steps', 'onboarding_templates'];
    ...
    try {
      ...
      if (leadResult.rows.length > 0) {
        ...
        console.log(`   ✅ Lead 1 found: ${lead.client_name} (Status: ${lead.status})`);
        console.log(`   📋 Template ID: ${lead.template_id || 'NULL - No template assigned!'}`);
      } else {
    ...
    try {
      ...
      if (columnsResult.rows.length > 0) {
        ...
        columnsResult.rows.forEach(col => {
          const marker = col.column_name === 'probability_percent' ? '⭐' : '  ';
          console.log(`   ${marker} ${col.column_name} (${col.data_type})`);
        });
        
        const hasProbabilityPercent = columnsResult.rows.some(col => col.column_name === 'probability_percent');
        if (!hasProbabilityPercent) {
          console.log('   ❌ probability_percent column MISSING from template_steps!');
        }
      } else {
        console.log('   ❌ template_steps table not found or empty');
      }
    } catch (error) {
      console.log(`   ❌ Error checking template_steps structure: ${error.message}`);
    }
    console.log('');
    ...
    try {
      ...
      if (columnsResult.rows.length > 0) {
        ...
        const hasProbabilityPercent = columnsResult.rows.some(col => col.column_name === 'probability_percent');
        if (!hasProbabilityPercent) {
          console.log('   ❌ probability_percent column MISSING from lead_steps!');
        }
      } else {
        console.log('   ❌ lead_steps table not found or empty');
      }
    } catch (error) {
      console.log(`   ❌ Error checking lead_steps structure: ${error.message}`);
    }
    console.log('');
    ...
    try {
      ...
      if (leadStepsResult.rows.length > 0) {
        ...
        leadStepsResult.rows.forEach((step, index) => {
          console.log(`   ${index + 1}. ${step.name} (${step.status}) - ${step.probability_percent || 0}%`);
        });
      } else {
    ...
    if (leadTemplateResult.rows.length > 0 && leadTemplateResult.rows[0].template_id) {
      ...
      try {
        ...
        if (templateStepsResult.rows.length > 0) {
          console.log(`   ✅ Found ${templateStepsResult.rows.length} template steps:`);
          let totalProb = 0;
      ...
      console.log('7. ⚠️  Cannot check template steps - no template_id assigned to lead 1');
    }
    ...
    console.error('   This means the database connection is not working properly.');
  } finally {
    await pool.end();
  }
}
