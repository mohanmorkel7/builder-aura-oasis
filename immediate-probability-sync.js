...
async function immediateSync() {
  ...
  try {
    ...
    for (const template of templates.rows) {
      ...
      console.log(`Template ${template.template_id}: ${steps.rows.length} steps`);

      if (steps.rows.length === 2) {
        // Set 40% and 60% for clear distinction
        await pool.query("UPDATE template_steps SET probability_percent = 40 WHERE id = $1", [steps.rows[0].id]);
        await pool.query("UPDATE template_steps SET probability_percent = 60 WHERE id = $1", [steps.rows[1].id]);
        ...
        for (let i = 0; i < steps.rows.length; i++) {
          ...
          await pool.query("UPDATE template_steps SET probability_percent = $1 WHERE id = $2", [percent, steps.rows[i].id]);
    ...
    for (const lead of leadsToSync.rows) {
      ...
      for (const leadStep of leadSteps.rows) {
        ...
        if (matchingTemplate) {
          ...
          console.log(`  📊 "${leadStep.name}": synced to ${matchingTemplate.probability_percent}%`);
        }
      }
      ...
      const newLeadProb = totalProb > 0 ? Math.round((completedProb / totalProb) * 100) : 0;
      
      await pool.query("UPDATE leads SET probability = $1 WHERE id = $2", [newLeadProb, lead.id]);
    ...
    if (testLead.rows.length > 0) {
      ...
      if (calculated !== lead.probability) {
        await pool.query("UPDATE leads SET probability = $1 WHERE id = $2", [calculated, lead.id]);
