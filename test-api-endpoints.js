...
async function testEndpoint(method, endpoint, body = null) {
  try {
    ...
    if (response.ok) {
      ...
      console.log(`❌ ${method} ${endpoint} - Status: ${response.status}, Error: ${data.error || 'Unknown error'}`);
...
async function runTests() {
  ...
  const tests = [
    ...
    { method: "GET", endpoint: "/clients/stats", description: "Get Client Stats" },
    ...
    { method: "GET", endpoint: "/leads/1/steps", description: "Get Lead Steps" },
    
    // Template endpoints
    { method: "GET", endpoint: "/templates", description: "Get All Templates" },
    
    // Deployment endpoints
    { method: "GET", endpoint: "/deployments", description: "Get All Deployments" },
    { method: "GET", endpoint: "/deployments/stats", description: "Get Deployment Stats" },
    { method: "GET", endpoint: "/deployments/products/list", description: "Get Products" },
    
    // Onboarding endpoints
    { method: "GET", endpoint: "/onboarding/clients/1/steps", description: "Get Client Onboarding Steps" },
    
    // File endpoints
    { method: "GET", endpoint: "/files/info/client_requirements.txt", description: "Get File Info" },
    
    // Chat endpoints
    { method: "GET", endpoint: "/leads/steps/1/chats", description: "Get Step Chats" },
    
    // Follow-up endpoints  
    { method: "GET", endpoint: "/follow-ups/lead/1", description: "Get Lead Follow-ups" },
    
    // Auth endpoint (should fail without credentials)
    { method: "POST", endpoint: "/users/auth/login", body: { email: "test@test.com", password: "wrong" }, description: "Login Test (should fail)" },
  ];
