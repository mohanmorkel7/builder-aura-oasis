...
async function checkDatabaseStatus() {
  ...
  try {
    ...
    if (tablesResult.rows.length > 0) {
      ...
      console.log('⚠️  No tables found. Database schema may need to be initialized.');
    }
    ...
    if (finopsTablesResult.rows.length > 0) {
      ...
      console.log('⚠️  No FinOps tables found. FinOps schema may need to be created.');
    }
    
    // Test SLA function if it exists
    console.log('\n⚙️  Testing SLA Function...');
    try {
      const slaResult = await client.query('SELECT * FROM check_subtask_sla_notifications() LIMIT 1');
