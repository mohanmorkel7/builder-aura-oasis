...
async function testMockProbability() {
  try {
    ...
    const updateResponse = await fetch('http://localhost:8080/api/leads/steps/3', {
      method: 'PUT',
      headers: {
    ...
    const updatedLeadResponse = await fetch('http://localhost:8080/api/leads/1');
    if (updatedLeadResponse.ok) {
      const updatedLeadData = await updatedLeadResponse.json();
      console.log(`✅ Updated Lead Probability: ${updatedLeadData.probability}%`);
    }

    // Get updated steps
    const updatedStepsResponse = await fetch('http://localhost:8080/api/leads/1/steps');
    if (updatedStepsResponse.ok) {
      ...
      const newExpectedProb = newTotalProb > 0 ? Math.round((newCompletedProb / newTotalProb) * 100) : 0;
