...
async function testTemplateLeadProbabilities() {
  ...
  try {
    ...
    for (const template of templates.rows) {
      ...
      console.log(`  Steps: ${template.step_count}, Total: ${template.total_probability || 0}%`);
    ...
    for (const lead of leads.rows) {
      ...
      const expectedPercentage = total > 0 ? Math.round((calculated / total) * 100) : 0;
      console.log(`  Expected Calculation: ${expectedPercentage}%`);
      console.log(`  Match: ${expectedPercentage === lead.probability ? '✅' : '❌'}`);
      console.log('');
    }
    ...
    try {
      ...
      if (stepsResponse.ok) {
        ...
        const stepsProbTotal = stepsData.reduce((sum, step) => sum + (step.probability_percent || 0), 0);
