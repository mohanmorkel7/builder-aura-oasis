...
async function fixLeadProbabilityDisplay() {
  try {
    ...
    try {
      ...
      console.log("The system will use mock data instead of real template data");
      return;
    }
    ...
    for (const template of templatesQuery.rows) {
      ...
      console.log(`  ${status} Template ${template.id}: "${template.name}" - ${template.total_probability}%`);
      
      if (template.total_probability !== 100 && template.step_count > 0) {
        console.log(`    🔄 Fixing template ${template.id} probability distribution...`);
        ...
        if (steps.rows.length === 2) {
          ...
          await pool.query('UPDATE template_steps SET probability_percent = 10 WHERE id = $1', [steps.rows[0].id]);
          await pool.query('UPDATE template_steps SET probability_percent = 90 WHERE id = $1', [steps.rows[1].id]);
          console.log(`    📊 Updated: ${steps.rows[0].name} = 10%, ${steps.rows[1].name} = 90%`);
          ...
          console.log(`    📊 Applied equal distribution: ${equalShare}% each (${remainder}% extra to first step)`);
        }
      }
    }
    ...
    console.log(`Found ${leadsWithSteps.rows.length} leads with steps needing probability sync`);
    ...
    for (const lead of leadsWithSteps.rows) {
      ...
      for (const leadStep of leadSteps.rows) {
        ...
        if (matchingTemplate && matchingTemplate.probability_percent) {
          ...
          console.log(`    📊 "${leadStep.name}": 0% → ${matchingTemplate.probability_percent}%`);
          totalUpdated++;
        }
      }
    }
    
    console.log(`✅ Updated ${totalUpdated} lead steps with template probabilities`);
    ...
    try {
      ...
      // Test lead 1 steps
      const leadResponse = await fetch("http://localhost:8080/api/leads/1/steps");
      if (leadResponse.ok) {
        const leadSteps = await leadResponse.json();
        console.log(`✅ Lead 1 API: ${leadSteps.length} steps returned`);
        
        const totalProb = leadSteps.reduce((sum, step) => sum + (step.probability_percent || 0), 0);
      ...
      if (templateResponse.ok) {
        ...
        console.log(`✅ Template 1 API: ${templateData.steps?.length || 0} steps returned`);
        
        if (templateData.steps) {
          const templateTotal = templateData.steps.reduce((sum, step) => sum + (step.probability_percent || 0), 0);
    ...
    console.log("2. Check that lead steps show correct probability percentages");
    console.log("3. Verify that 'Add New Step' modal shows template steps with probabilities");
    console.log("4. Test status changes to see if lead probability updates automatically");
